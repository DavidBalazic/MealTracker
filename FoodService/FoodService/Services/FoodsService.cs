...
namespace FoodService.Services
{
    public class FoodsService
    {
        ...
        public async Task<List<Food>> GetByExcludedAllergensAsync(List<string> excludedAllergens) => await _foodsCollection.Find(f => !f.Allergens.Any(allergen => excludedAllergens.Contains(allergen))).ToListAsync();
        // POST
        ...
        {
            return await _foodsCollection.DeleteManyAsync(f => f.Name.Contains(name));
        }
    }
}